        ...
        function constrainXY(toX, toY, toScale, panOptions) {
            ...
            if (opts.contain) {
                ...
                if (opts.contain === 'inside') {
                    ...
                    var dims_1 = getDimensions(elem);
                    var realWidth_1 = dims_1.elem.width / scale;
                    var realHeight_1 = dims_1.elem.height / scale;
                    var scaledWidth_1 = realWidth_1 * toScale;
                    var scaledHeight_1 = realHeight_1 * toScale;
                    var diffHorizontal_1 = (scaledWidth_1 - realWidth_1) / 2;
                    var diffVertical_1 = (scaledHeight_1 - realHeight_1) / 2;
                    if (scaledWidth_1 < dims_1.parent.width) {
                        result.x =
                            (dims_1.parent.width / 2 -
                                scaledWidth_1 / 2 -
                                dims_1.parent.padding.left -
                                dims_1.parent.border.left +
                                diffHorizontal_1) /
                                toScale;
                    }
                    else {
                        var minX = (-(scaledWidth_1 - dims_1.parent.width) -
                            dims_1.parent.padding.left -
                            dims_1.parent.border.left -
                            dims_1.parent.border.right +
                            diffHorizontal_1) /
                            toScale;
                        var maxX = (diffHorizontal_1 - dims_1.parent.padding.left) / toScale;
                        result.x = Math.max(Math.min(result.x, maxX), minX);
                    }
                    if (scaledHeight_1 < dims_1.parent.height) {
                        result.y =
                            (dims_1.parent.height / 2 -
                                scaledHeight_1 / 2 -
                                dims_1.parent.padding.top -
                                dims_1.parent.border.top +
                                diffVertical_1) /
                                toScale;
                    }
                    else {
                        var minY = (-(scaledHeight_1 - dims_1.parent.height) -
                            dims_1.parent.padding.top -
                            dims_1.parent.border.top -
                            dims_1.parent.border.bottom +
                            diffVertical_1) /
                            toScale;
                        var maxY = (diffVertical_1 - dims_1.parent.padding.top) / toScale;
                        result.y = Math.max(Math.min(result.y, maxY), minY);
                    }
                }
            }
